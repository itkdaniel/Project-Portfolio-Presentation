...
Why BPE?
  - Handles OOV words by falling back to subwords / characters
  - Vocabulary size is controllable
  - Good balance between character and word-level encoding
  - Used in GPT-2, RoBERTa, LLaMA
...
END_OF_WORD    = "##"   # Marker appended to end of word (WordPiece style)
...
def _word_to_chars(word: str) -> Tuple[str, ...]:
    """Split word into characters, appending end-of-word to last char."""
    chars = list(word)
...
def _get_pair_counts(vocab: Dict[Tuple[str, ...], int]) -> Counter:
    """Count all adjacent bigrams across all words weighted by frequency."""
...
class BPETokenizer:
    ...
    def train(self, texts: List[str], num_merges: Optional[int] = None) -> None:
        ...
        Steps:
          1. Build character-level frequency dictionary
          2. Iteratively find + apply the most frequent pair merge
          3. Add merged tokens to vocabulary

        Complexity: O(num_merges × |vocab| × avg_word_len)
        """
        num_merges = num_merges or (self.max_vocab - len(SPECIAL_TOKENS) - 256)

        # Step 1: Build word frequency counter from corpus
        word_freq: Counter = Counter()
        for text in texts:
            for word in re.findall(r"[a-zA-Z0-9']+", text.lower()):
                word_freq[word] += 1

        # Step 2: Initialize char-level vocab
    ...
    def _tokenize_word(self, word: str) -> List[str]:
        """Apply learned merges to a single word greedily."""
